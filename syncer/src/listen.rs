                ...
                Ok(event) => {
                    match event.kind {
                        EventKind::Create(CreateKind::File) => {
                            match accept_single_path(event) {
                                Ok(path) => {
                                    (self.callback)(path);
                                }
                                Err(reason) => {
                        ...
                        EventKind::Access(_) | EventKind::Remove(_) | EventKind::Modify(ModifyKind::Metadata(_)) => {
...
fn accept_single_path<'a>(mut event: Event) -> Result<PathBuf, String> {
    ...
    match event.paths.len() {
        1 => {
            Ok(event.paths.remove(0))
        }
        0 => {
            Err(format!("no-path create event: {event:?}"))
        }
        _ => {
            Err(format!("multiple-path create event: {event:?}"))
        }
    }
}
